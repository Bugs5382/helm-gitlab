...
NOTE (umbrella fork): renamed from `common.image` to `valkey.common.image`
to avoid a chart-helper name collision with the `seaweedfs` subchart in
this umbrella, which also defines a global `common.image` template with
an incompatible signature. Helm registers named templates globally per
release; the second definition silently shadows the first, so when both
charts ship the same name the cross-chart render breaks. See
docs/upstream-bug-reports/valkey-helm-0.9.4-helper-collision.md.
*/}}
